		...
		<div class="container-fluid">
			...
			<div>
				<ul class="nav navbar-nav">
        			...
        			<li class="active"><a href="calendar.php"> Kalendar </a></li>
      			</ul>
      			<ul class="nav navbar-nav navbar-right">
        			<li> <a href="welcome.php"> <span class="glyphicon glyphicon-user"> </span> <?php if  (isset($_SESSION["user"])) echo $_SESSION["user"]; else echo "Gost"; ?> </a> </li>
...
function draw_schedule(){
    $raspored = '<table cellpadding="0" cellspacing="0" class="calendar">';
     ...
     for($x = 8; $x < 21; $x++) {
        ...
        for($y = 0; $y < 6; $y++) {
           // ispisi raspored za predavaonu y
                $ispis = "";
               // ovisno o tome je li korisnik ulogiran, pridruzi varijabli ispis slobodno ili name2show
...
if (isset($_GET['r']) && isset($_GET['h']) && isset($_GET['a'])) {
    ...
    if ($akcija) {
      $id = rand();
      if (insert_into_reservations ($ucione[$pred], $datum, $sat, $_SESSION["user"], $id))
...
// ako je odabran dan, nacrtaj tablicu sa predavaonama
	...
	function dalje(val, m, g) {
    	...
    	if (val.value == 'Natrag') {
        	if (( ((m-1)>0) && (g==2016)) || ( (m>8) && (g==2015) )) {
            	xhr1.open( "GET", "calendar.php?" + "m=" + (m-1) + "&g=" + g, true );
            	mj = m-1; god = g; }
        	else if ((m-1)<1) {
                xhr1.open( "GET", "calendar.php?" + "m=" + 12 + "&g=" + (g-1), true );
                mj = 12; god = g-1; }
            else {
                xhr1.open( "GET", "calendar.php?" + "m=" + m + "&g=" + g, true );
                mj = m; god = g; }
        }
        	...
        	if (( (m<12) && (g==2015) ) || ( (m<7) && (g==2016) )) {
                ...
                xhr1.open( "GET", "calendar.php?" + "m=" + 1 + "&g=" + (g+1), true );
                mj = 1; god = g+1; }
            else {
                 xhr1.open( "GET", "calendar.php?" + "m=" + m + "&g=" + g, true );
                 mj = m; god = g; }
	...
	function predavaone(val) {
    	...
    	xhr2.open( "GET", "calendar.php?d=" + val, true );
    	xhr2.send();
	}
...
function obradi(pred, sat, sto) {
    ...
    if (sto)
        xhr3.open( "GET", "calendar.php?r=" + pred + "&h=" + sat + "&a=1", true );
    else
        xhr3.open( "GET", "calendar.php?r=" + pred + "&h=" + sat + "&a=0", true );
    xhr3.send();
}
...
function ispisi (sto) {
    ...
    if (sto.value == 'Da')
        xhr4.open( "GET", "calendar.php?d=" + dan, true );
    else
    // vratit ce ga na trenutni mjesec
        xhr4.open( "GET", "calendar.php?m=" + mj + "&g=" + god, true );
    xhr4.send();
}
